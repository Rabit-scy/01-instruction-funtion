...
void QuickSort(int low, int high) {
  /* 2. The Caller stores a return address and the old value of top-sp into the Callee's frame. */
  /* the Caller decrements top-sp. */
  /* top-sp is moved    past the caller's local data and temporaries      and     the callee's parameters and status fields. */
  6a:	1101                	addi	sp,sp,-32
  6c:	ce06                	sw	ra,28(sp)
  6e:	cc22                	sw	s0,24(sp)
  70:	1000                	addi	s0,sp,32
  /* 3.The callee saves the register values and other status infomation */
  72:	fea42223          	sw	a0,-28(s0)    // argument low
  76:	feb42023          	sw	a1,-32(s0)    // arg high
  /* 4. The Callee initializes its local data and begins execution  */ 
  int i = low;
  7a:	fe442783          	lw	a5,-28(s0)
  7e:	fef42823          	sw	a5,-16(s0)    // local var i
  int j = high;
  82:	fe042783          	lw	a5,-32(s0)
  86:	fef42623          	sw	a5,-20(s0)    // local var j
  if (i >= j) {
  ...
  9a:	fe442703          	lw	a4,-28(s0)  // low 
  9e:	070a                	slli	a4,a4,0x2
  a0:	00078793          	mv	a5,a5
  a4:	97ba                	add	a5,a5,a4
  /* need_to_sort[low] */
  a6:	439c                	lw	a5,0(a5)
  /* temp =  save local var--temp to stack ,same as 4. */
  a8:	fef42423          	sw	a5,-24(s0)

  /* Callee funtion "main body" */
  while (i != j) {
...
4:
  ...
  ce:	00e7ce63          	blt	a5,a4,ea <QuickSort+0x80>   /* 1f */
  d2:	ff042703          	lw	a4,-16(s0)
  d6:	fec42783          	lw	a5,-20(s0)
  da:	fcf74ae3          	blt	a4,a5,ae <QuickSort+0x44>   /* 1b */
    }
    while (need_to_sort[i] <= temp && i < j) {
  de:	a031                	j	ea <QuickSort+0x80>     /* 1f */
2:
      i++;
...
1:
 ...
 100:	00f74863          	blt	a4,a5,110 <__stack_size+0x10>   /* 2f */
 104:	ff042703          	lw	a4,-16(s0)
 108:	fec42783          	lw	a5,-20(s0)
 10c:	fcf74ae3          	blt	a4,a5,e0 <QuickSort+0x76>       /* 2b */
    }
2:
    if (i < j) {
 110:	ff042703          	lw	a4,-16(s0)
 114:	fec42783          	lw	a5,-20(s0)
 118:	02f75863          	bge	a4,a5,148 <__stack_size+0x48>   /* 3f */

      swap(&need_to_sort[i], &need_to_sort[j]);
  /* 1-1.value communicated between caller and callee */
 ...
 140:	97ba                	add	a5,a5,a4
  /* 1-1. use a0, a1 for communication. */
 142:	85be                	mv	a1,a5
 144:	8536                	mv	a0,a3
...
3:
 ...
 150:	f6f714e3          	bne	a4,a5,b8 <QuickSort+0x4e>   /* 4b */
    }
  }
...
int main(void) {
 ...
 1ae:	0060                	addi	s0,sp,12
  QuickSort(0, 11);     /* 1. the caller evaluates the actual parameters */
 1b0:	45ad                	li	a1,11
 1b2:	4501                	li	a0,0
